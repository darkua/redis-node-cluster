...
RedisCluster.prototype.send_cluster_command = function(command,args,callback) {
  ...
  //execute the right command type
  this[command.type](command.action,args,callback);
};

RedisCluster.prototype.command_all = function(command,args,callback) {
  var that = this;
  var nodes = this.startup_nodes.slice(); //copy startup nodes!
  
  // Just the for the insanity of beliving we can do it wihtout async!
  var acc0 = {
    ready : false,
    ids : [],
    add : function(id){
    ...
    cb:function(id){
      ...
      console.log('count acc0',id,this.ids);
  ...
  var acc ={
    ...
    cb:function(id,err,res){
      if(typeof id === "number"){
        this.ids.splice(this.ids.indexOf(id),1);
        console.log('count acc1',id,this.ids);
        if(this.ids.length == 0)
          this.ready = true;
        this.err.push(err);
        this.res.push(res);
  ...
  nodes.forEach(function(n,i){
    var c = that.connections[n.name] || that.get_redis_link(n.host,n.port);
    acc0.add(i);
    c.info(["Replication"],function(i,err,res){
        var match = /role:master/.exec(res)
        if(match){
          console.log(c.port,i);
          acc.add(i);
          c.send_command(command, args,acc.cb.bind(acc,i));
        }
      acc0.cb.call(acc0,i);
    }.bind(null,i));
  })
...
RedisCluster.prototype.command_any = function(command,args,callback) {
  //execute command if cluster is ready, or store in queue
  if(!this.cluster_ready){
    this.queue.push(this.send_cluster_command_any.bind(this,command,args,callback))
    return;
  }
  ...
  this.get_random_connection(function(err,_node,c){
    if(!err){
      c.send_command(command, args,callback);  
    }
  })
}


RedisCluster.prototype.command_sub = function(command,args,callback) {
  var that= this;
...
RedisCluster.prototype.command = function(command,args,callback,ttl) {
  ...
  if(ttl>0){
    ...
    this.get_connection_by_slot(slot,function(err,client){
      if(err){ 
        console.log('COMMAND ERR',command,'client',client.port,client.pub_sub_mode);
        callback(err,null);
      }else{
        console.log('COMMAND',command,'client',client.port,client.pub_sub_mode);
...
**/
RedisCluster.prototype.disconnect = function(cb) {
  var that =this;
  ...
  if(typeof cb !== "undefined")
    cb(); //#todo calling cb on end events!
};
...
var inAllCommands = ["flushdb","flushall","dbsize"]
var inAnyCommands = ["time"];
var inSubCommands = ["subscribe","psubscribe","unsubscribe","punsubscribe"];

function getCommandType(command){
  if(inAllCommands.indexOf(command)!==-1)
    return "command_all";
  if(inAnyCommands.indexOf(command)!==-1)
    return "command_any";
  if(inSubCommands.indexOf(command)!==-1)
    return "command_sub";
...
commands.forEach(function (fullCommand) {
    ...
    var send_command = null;

    if(notValidCommands.indexOf(command)==-1){
      //valid commands
      //check type on assignment
      RedisCluster.prototype[command] = function (args, callback) {
        if (Array.isArray(args) && ((typeof callback === "function") || (typeof callback === "undefined"))){
          return this.send_cluster_command({type:getCommandType(command),action:command}, args, callback);
        } else {
          return this.send_cluster_command({type:getCommandType(command),action:command}, Array.prototype.slice.call(arguments));
        };
      };
    }else{
      //not valid commands in cluster mode, throw error!
      var nil = function(){this.emit("error",new Error(command+" not supported in cluster mode!"))};
      RedisCluster.prototype[command] = nil;
    }
    //UPPER CASE COMMANDS 
...
RedisCluster.prototype.AUTH = RedisCluster.prototype.auth;
