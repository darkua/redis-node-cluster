...
var notValidCommands = ["info","multi","exec","slaveof","config","shutdown"];
...
RedisCluster.prototype.get_redis_link = function(host,port) {
  ...
  c.on("error",function(err){
    that.emit("error", err); //pass the error to the cluster error handler!
  });
  return c;
}
...
RedisCluster.prototype.get_random_connection = function(cb) {
  ...
  function iterateNodes(n){
    console.log('interate_random',nodes,n);
    if(n){
      ...
      c.ping(function(err,res){
        if(err || res !== "PONG"){
          ...
          that.close_existing_connection();
          that.connections[n.name] = c;
          cb(null,c);
        }
      });
    } else {
...
RedisCluster.prototype.get_connection_by_slot = function(slot,cb) {
  ...
  if(!node)
    this.get_random_connection(cb);
  else{
    node = this.set_node_name(node);
    if(!this.connections[node.name]){
      this.close_existing_connection();
      this.connections[node.name] = this.get_redis_link(node.host,node.port);
    } //#TODO exeption on close connections or creating new one!
    cb(null,this.connections[node.name]);
  }
};
/**
...
RedisCluster.prototype.send_cluster_command = function(command,args,callback,ttl) {
  var that = this;
  
  //execute command if cluster is ready, or store in queue
  if(!this.cluster_ready){
    this.queue.push(this.send_cluster_command.bind(this,command,args,callback,ttl))
    return;
  }
  ...
  function wrap_moved_callback(err,res){
    if(err){
      ...
      if(moved[0] == "MOVED" || moved[0] == "ASK" ){ 
        ...
        that.send_cluster_command(command,args,callback,--ttl);
      } else{
